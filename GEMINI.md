...
category: z.enum(['GCP', 'Workspace', 'IA', 'ARM Reversing'])
author: z.string().default('DakyLabs')
draft: z.boolean().default(false)
```

Nuevo artículo: copiar `src/content/blog/_plantilla.md`, nombre en **kebab-case**, `draft: false` para publicar.
...
GitHub Pages desde `/dist`. El archivo `dist/CNAME` contiene `dakylabs.com` — no eliminarlo.

## Tono del contenido

Blog en **español**, voz directa y técnica. Ecosistema **Google Cloud**: arquitectura, automatización y ML. Dirigido a la comunidad técnica latinoamericana.

## Lo que NO hacer

- No instalar dependencias sin consultar primero.
- No modificar `dist/` directamente — lo genera `astro build`.
- No crear archivos `.md` adicionales sin que se pidan explícitamente.
- No añadir animaciones o estilos decorativos no solicitados.
